...
static IScaleLayer *addBatchNorm2d(INetworkDefinition *network, std::map<std::string, Weights> &weightMap, ITensor &input, std::string lname, float eps)
{
    ...
    Weights scal{DataType::kFLOAT, scval, len};
    ...
    weightMap[lname + ".scale"] = scal;
    weightMap[lname + ".shift"] = shift;
    weightMap[lname + ".power"] = power;
    IScaleLayer *scale_1 = network->addScale(input, ScaleMode::kCHANNEL, shift, scal, power);
    assert(scale_1);
    return scale_1;
}
